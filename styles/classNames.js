export const titleName = "Thang Codes";

export const maxWidth = "max-w-[5000px]";

export const bgDefault = "bg-gradient-to-r from-green-600 to-green-500";
export const bgDefault2 = "bg-gradient-to-r from-green-500 to-green-600";
export const textDefault = "text-green-600 dark:text-green-400";
export const bgWelcome =
  "bg-gradient-to-br from-green-400 via-green-500 to-green-600 dark:from-green-700 dark:via-green-700 dark:to-green-800";
export const buttonDefault = "bg-green-500 hover:bg-green-600";
