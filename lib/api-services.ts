...
export const callRegister = (userData: {
  ...
  return apiClient.post<IBackendRes<{ user: IUser; access_token: string }>>("/api/v1/auth/register", userData)
}

export const callLogin = (credentials: { email: string; password: string }) => {
  return apiClient.post<IBackendRes<{ user: IUser; access_token: string }>>("/api/v1/auth/login", credentials)
}
...
export const callRefreshToken = () => {
  return apiClient.get<IBackendRes<{ access_token: string }>>("/api/v1/auth/refresh")
}

export const callForgotPassword = (email: string) => {
  return apiClient.post<IBackendRes<string>>("/api/v1/auth/forgot-password", { email })
}
...
export const callFetchPosts = (query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<IPost>>>(`/api/v1/posts?${query}`)
}
...
export const callFetchRelatedPosts = (postId: string, limit = 5) => {
  return apiClient.get<IBackendRes<IPost[]>>(`/api/v1/posts/${postId}/related?limit=${limit}`)
}

// Lấy bài viết nổi bật - Trang chủ (/)
export const callFetchFeaturedPosts = (limit = 6) => {
  return apiClient.get<IBackendRes<IPost[]>>(`/api/v1/posts/featured?limit=${limit}`)
}

// Lấy bài viết mới nhất - Trang chủ (/)
export const callFetchLatestPosts = (limit = 10) => {
  return apiClient.get<IBackendRes<IPost[]>>(`/api/v1/posts/latest?limit=${limit}`)
}

// Lấy bài viết trending - Trang chủ (/)
export const callFetchTrendingPosts = (limit = 4) => {
  return apiClient.get<IBackendRes<IPost[]>>(`/api/v1/posts/trending?limit=${limit}`)
}
...
export const callSearchPosts = (
  ...
  return apiClient.get<IBackendRes<IModelPaginate<IPost>>>(`/api/v1/posts/search?${params.toString()}`)
}

// Tăng view count - Trang chi tiết bài viết
export const callIncrementPostView = (postId: string) => {
  return apiClient.patch<IBackendRes<{ views: number }>>(`/api/v1/posts/${postId}/view`)
}

// Like/Unlike bài viết - Trang chi tiết bài viết
export const callTogglePostLike = (postId: string) => {
  return apiClient.patch<IBackendRes<{ likes: number; isLiked: boolean }>>(`/api/v1/posts/${postId}/like`)
}
...
export const callFetchCategories = (query?: string) => {
  return apiClient.get<IBackendRes<ICategory[]>>(`/api/v1/categories${query ? `?${query}` : ""}`)
}

// Lấy danh mục theo ID - Trang danh mục chi tiết (/categories/[id])
export const callFetchCategoryById = (id: string) => {
  return apiClient.get<IBackendRes<ICategory>>(`/api/v1/categories/${id}`)
}

// Lấy danh mục theo slug - Trang danh mục chi tiết (/categories/[slug])
export const callFetchCategoryBySlug = (slug: string) => {
  return apiClient.get<IBackendRes<ICategory>>(`/api/v1/categories/slug/${slug}`)
}
...
export const callFetchPostsByCategory = (categoryId: string, query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<IPost>>>(`/api/v1/categories/${categoryId}/posts?${query}`)
}

// Lấy danh mục phổ biến - Sidebar, Footer
export const callFetchPopularCategories = (limit = 10) => {
  return apiClient.get<IBackendRes<ICategory[]>>(`/api/v1/categories/popular?limit=${limit}`)
}
...
export const callFetchTags = (query?: string) => {
  return apiClient.get<IBackendRes<ITag[]>>(`/api/v1/tags${query ? `?${query}` : ""}`)
}
...
export const callFetchPostsByTag = (tagId: string, query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<IPost>>>(`/api/v1/tags/${tagId}/posts?${query}`)
}

// Lấy tags phổ biến - Sidebar
export const callFetchPopularTags = (limit = 20) => {
  return apiClient.get<IBackendRes<ITag[]>>(`/api/v1/tags/popular?limit=${limit}`)
}
...
export const callUpdateComment = (commentId: string, content: string) => {
  return apiClient.patch<IBackendRes<IComment>>(`/api/v1/comments/${commentId}`, {
    content,
  })
}
...
export const callToggleCommentLike = (commentId: string) => {
  return apiClient.patch<IBackendRes<{ likes: number; isLiked: boolean }>>(`/api/v1/comments/${commentId}/like`)
}
...
export const callFetchPostsByUser = (userId: string, query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<IPost>>>(`/api/v1/users/${userId}/posts?${query}`)
}

// Cập nhật avatar - Trang profile
export const callUpdateAvatar = (formData: FormData) => {
  return apiClient.patch<IBackendRes<{ avatar: string }>>("/api/v1/users/avatar", formData, {
    headers: {
...
export const callSubscribeNewsletter = (email: string) => {
  return apiClient.post<IBackendRes<INewsletter>>("/api/v1/newsletter/subscribe", {
    email,
  })
}
...
export const callConfirmNewsletter = (token: string) => {
  return apiClient.get<IBackendRes<string>>(`/api/v1/newsletter/confirm/${token}`)
}
...
export const callTrackPageView = (page: string, metadata?: Record<string, any>) => {
...
export const callTrackPostView = (postId: string, metadata?: Record<string, any>) => {
  return apiClient.post<IBackendRes<string>>("/api/v1/analytics/track", {
    type: "post_view",
    resource: postId,
    metadata,
...
export const callUpdatePost = (postId: string, postData: Partial<IPost>) => {
  return apiClient.patch<IBackendRes<IPost>>(`/api/v1/admin/posts/${postId}`, postData)
}
...
export const callCreateCategory = (categoryData: Partial<ICategory>) => {
  return apiClient.post<IBackendRes<ICategory>>("/api/v1/admin/categories", categoryData)
}

// Cập nhật danh mục - Admin panel
export const callUpdateCategory = (categoryId: string, categoryData: Partial<ICategory>) => {
  return apiClient.patch<IBackendRes<ICategory>>(`/api/v1/admin/categories/${categoryId}`, categoryData)
}

// Xóa danh mục - Admin panel
export const callDeleteCategory = (categoryId: string) => {
  return apiClient.delete<IBackendRes<string>>(`/api/v1/admin/categories/${categoryId}`)
}

// Quản lý comments - Admin panel
export const callFetchAllComments = (query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<IComment>>>(`/api/v1/admin/comments?${query}`)
}

// Duyệt comment - Admin panel
export const callApproveComment = (commentId: string) => {
  return apiClient.patch<IBackendRes<IComment>>(`/api/v1/admin/comments/${commentId}/approve`)
}

// Từ chối comment - Admin panel
export const callRejectComment = (commentId: string) => {
  return apiClient.patch<IBackendRes<IComment>>(`/api/v1/admin/comments/${commentId}/reject`)
}

// Quản lý newsletter subscribers - Admin panel
export const callFetchNewsletterSubscribers = (query: string) => {
  return apiClient.get<IBackendRes<IModelPaginate<INewsletter>>>(`/api/v1/admin/newsletter?${query}`)
}
...
export const callSendNewsletter = (newsletterData: {
  ...
  return apiClient.post<IBackendRes<string>>("/api/v1/admin/newsletter/send", newsletterData)
}
